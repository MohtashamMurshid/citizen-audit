...
export const overallStats = query({
  ...
  handler: async (ctx) => {
    ...
    return {
      ...
      needsReview: reports.filter((r) => r.verdict === "needs_review").length,
  ...
  handler: async (ctx) => {
    ...
    return Object.entries(cityMap)
      .map(([city, stats]) => ({
        city,
        ...stats,
        accuracy: stats.total > 0 ? stats.verified / stats.total : 0,
      }))
      .sort((a, b) => b.total - a.total);
  },
});

export const stateLeaderboard = query({
  args: {},
  handler: async (ctx) => {
    const reports = await ctx.db.query("reports").collect();
    ...
    for (const r of reports) {
      if (!stateMap[r.state]) {
        stateMap[r.state] = { total: 0, suspicious: 0, verified: 0, notFound: 0 };
      }
      stateMap[r.state].total++;
      if (r.verdict === "suspicious" || r.verdict === "needs_review") {
        stateMap[r.state].suspicious++;
...
export const scanMilestones = query({
  args: {},
  handler: async (ctx) => {
    const reports = await ctx.db.query("reports").collect();
    const total = reports.length;
    const cities = new Set(reports.map((r) => r.city)).size;
    const states = new Set(reports.map((r) => r.state)).size;
    const suspicious = reports.filter(
      (r) => r.verdict === "suspicious" || r.verdict === "needs_review"
    ).length;
    const verified = reports.filter((r) => r.verdict === "verified").length;

    return {
      totalScans: total,
      citiesCovered: cities,
      statesCovered: states,
      suspiciousFlagged: suspicious,
